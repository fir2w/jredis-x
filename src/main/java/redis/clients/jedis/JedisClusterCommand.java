...
public abstract class JedisClusterCommand<T> {
    ...
    public T runAllConnection(String key) {
        if (key == null) {
            throw new JedisClusterException("No way to dispatch this command to Redis Cluster.");
        }
        T ret = null;
        List<Jedis> jedisList = connectionHandler.queryEffectiveConnection();
        for (Jedis connection : jedisList) {
            ret = runWithByConnection(key, connection);
        }
        return ret;
    }

    private T runWithByConnection(String key, Jedis connection) {
        if (redirections <= 0) {
            throw new JedisClusterMaxRedirectionsException("Too many Cluster redirections?");
        }
        try {
            ...
            // release current connection before recursion
            System.out.println("runWithByConnection exception :" + jce.getMessage());
                    releaseConnection(connection);
            connection = null;
