...
public class JedisCluster implements JedisCommands, BasicCommands, Closeable {
    ...
    @Override
    public Long flushLikeKey(final String likeKey) {
        return new JedisClusterCommand<Long>(connectionHandler, maxRedirections) {
            @Override
            public Long execute(Jedis connection) {
                System.out.println(connection.getClient().getSocket() + " 端口:" + connection.getClient().getPort() + connection.dataSource.internalPool);
                Long rLong = 0l;
                String pattern = likeKey + "*";
                //pattern = "index*";
                Set<String> set = connection.keys(pattern);
                for (String key : set) {
                    System.out.println(key);
                    try {
                        rLong = connection.del(key);
                    }catch(Exception e){
                        e.printStackTrace();
                    }
                }
